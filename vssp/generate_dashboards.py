...
def main() -> int:
    ...
    # EN | and a size token's percentage into a calc() multiplier.
    ...
    for dash_id, tpl_name, out_name, extra in jobs:
        ...
        # EN | ON-DEMAND DASHBOARDS STAY OFF UNTIL ASKED FOR. home, core and
        # EN | energy are created from the ADMIN console and can be
        # EN | deactivated there, which deletes their view file. A routine run
        # EN | (no --only: ROOMS sync, ASSIGN save, REGENERATE ALL, the CI
        # EN | pod-side regeneration) used to RECREATE them from the model, so
        # EN | a deactivation lasted until the next time anything at all was
        # EN | regenerated - going into ROOMS & FLOORS brought all three back.
        # EN | Absence is the state, the same truth the presence sensors and
        # EN | the nav rail already read: an absent on-demand dashboard is
        # EN | refreshed by nobody and created only by the caller that names
        # EN | it in --only, i.e. the CREATE button. An existing one is still
        # EN | regenerated by every routine run, so it never goes stale.
        # FR | LES DASHBOARDS A LA DEMANDE RESTENT ETEINTS TANT QU ON NE LES
        # FR | DEMANDE PAS. home, core et energy se creent depuis la console
        # FR | ADMIN et peuvent y etre desactives, ce qui supprime leur
        # FR | fichier de vue. Un passage de routine (sans --only : sync
        # FR | ROOMS, enregistrement ASSIGN, REGENERER TOUT, la regeneration
        # FR | cote pod de la CI) les RECREAIT depuis le modele, donc une
        # FR | desactivation ne tenait que jusqu a la prochaine regeneration,
        # FR | quelle qu elle soit - entrer dans ROOMS & FLOORS les ramenait
        # FR | tous les trois. L absence EST l etat, la meme verite que lisent
        # FR | deja les capteurs de presence et le bandeau de nav : un
        # FR | dashboard a la demande absent n est regenere par personne et
        # FR | n est cree que par l appelant qui le nomme dans --only, c est a
        # FR | dire le bouton CREER. Un dashboard present, lui, reste
        # FR | regenere par chaque passage de routine, donc jamais perime.
        if (dash_id in OPTIONAL_SYSTEM_NAV_IDS and not wanted
                and not (out_dir / f"{dash_id}.yaml").is_file()):
            print(f"= {dash_id} absent - dashboard a la demande, laisse "
                  f"eteint (bouton CREER {dash_id.upper()} de la console ADMIN)")
            status["skipped"].append(f"{dash_id} (on demand, not created)")
            continue
